...
Pushing a `v*` tag triggers two workflows: `release.yml` creates a GitHub Release whose body is read from the matching version section in `docs/CHANGELOG.md`, and `publish.yml` builds, tests, and publishes the package to npm. Add a `## [x.y.z]` section to the changelog before tagging.
